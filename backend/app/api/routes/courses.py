...
def list_lessons(course_id: int, membership: Membership = Depends(active_membership), tenant: Tenant = Depends(tenant_context), db: Session = Depends(get_db)) -> list[LessonRead]:
    ...
    lessons = db.scalars(
        query.order_by(Lesson.section_id.is_(None), Lesson.section_id, Lesson.sort_order, Lesson.id)
    ).all()
...
def reorder_lessons(
    ...
    _require_course_in_tenant(db, tenant.id, course_id)
    lessons = db.scalars(
        select(Lesson).where(Lesson.tenant_id == tenant.id, Lesson.course_id == course_id)
    ).all()
